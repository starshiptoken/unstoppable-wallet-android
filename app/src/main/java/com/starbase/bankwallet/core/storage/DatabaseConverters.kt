...
class DatabaseConverters {
    ...
    fun decryptSecretString(value: String?): SecretString? {
        ...
        return try {
            SecretString(com.starbase.bankwallet.core.App.encryptionManager.decrypt(value))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretString(secretString: SecretString?): String? {
        return secretString?.value?.let { com.starbase.bankwallet.core.App.encryptionManager.encrypt(it) }
    }
    ...
    fun decryptSecretList(value: String?): SecretList? {
        ...
        return try {
            SecretList(com.starbase.bankwallet.core.App.encryptionManager.decrypt(value).split(","))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretList(secretList: SecretList?): String? {
        return secretList?.list?.joinToString(separator = ",")?.let {
            com.starbase.bankwallet.core.App.encryptionManager.encrypt(it)
        }
    }
