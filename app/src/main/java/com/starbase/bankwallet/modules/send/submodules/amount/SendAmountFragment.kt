    ...
    private fun setValidationError(error: SendAmountModule.ValidationError?) {
        ...
        txtHintError.text = when (error) {
            ...
            is SendAmountModule.ValidationError.NotEnoughForMinimumRequiredBalance -> {
                context?.getString(R.string.Send_Error_MinRequiredBalance, com.starbase.bankwallet.core.App.numberFormatter.formatCoin(error.minimumRequiredBalance.value, error.minimumRequiredBalance.coin.code, 0, 8))
            }
            else -> null
        }
    }
