package com.starbase.bankwallet.modules.coin.adapters
...
class ChartViewHolder(override val containerView: View, private val listener: CoinChartAdapter.Listener, private val currency: Currency)
    ...
    private fun setSelectedPoint(item: ChartPointViewItem) {
        ...
        pointInfoPrice.text = com.starbase.bankwallet.core.App.numberFormatter.formatFiat(item.price.value, item.price.currency.symbol, 2, 4)
        ...
        item.macdInfo?.let { macdInfo ->
            macdInfo.histogram?.let {
                ...
                macdHistogram.text = com.starbase.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.signal?.let {
                macdSignal.isVisible = true
                macdSignal.text = com.starbase.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.macd?.let {
                macdValue.isVisible = true
                macdValue.text = com.starbase.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
        }
    }

    private fun formatFiatShortened(value: BigDecimal, symbol: String): String {
        val shortCapValue = com.starbase.bankwallet.core.App.numberFormatter.shortenValue(value)
        return com.starbase.bankwallet.core.App.numberFormatter.formatFiat(shortCapValue.first, symbol, 0, 2) + " " + shortCapValue.second
    }
