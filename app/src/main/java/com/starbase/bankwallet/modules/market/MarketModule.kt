...
object MarketModule {
    ...
    class Factory : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            val service = MarketService(com.starbase.bankwallet.core.App.marketStorage)
...
data class MarketViewItem(
    ...
    companion object {
        fun create(marketItem: MarketItem, marketField: MarketField): MarketViewItem {
            val formattedRate = com.starbase.bankwallet.core.App.numberFormatter.formatFiat(marketItem.rate.value, marketItem.rate.currency.symbol, 0, 6)

            val marketDataValue = when (marketField) {
                MarketField.MarketCap -> {
                    val marketCapFormatted = marketItem.marketCap?.let { marketCap ->
                        val (shortenValue, suffix) = com.starbase.bankwallet.core.App.numberFormatter.shortenValue(marketCap.value)
                        com.starbase.bankwallet.core.App.numberFormatter.formatFiat(shortenValue, marketCap.currency.symbol, 0, 2) + " $suffix"
                    }

                    MarketDataValue.MarketCap(marketCapFormatted ?: Translator.getString(R.string.NotAvailable))
                }
                MarketField.Volume -> {
                    val (shortenValue, suffix) = com.starbase.bankwallet.core.App.numberFormatter.shortenValue(marketItem.volume.value)
                    val volumeFormatted = com.starbase.bankwallet.core.App.numberFormatter.formatFiat(shortenValue, marketItem.volume.currency.symbol, 0, 2) + " $suffix"
