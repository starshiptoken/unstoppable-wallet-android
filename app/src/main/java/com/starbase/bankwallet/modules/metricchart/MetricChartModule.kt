package com.starbase.bankwallet.modules.metricchart
    ...
    class Factory(private val metricsChartType: MetricChartType?) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            val fetcher = when (val type = metricsChartType) {
                is MetricChartType.Coin -> CoinTvlFetcher(com.starbase.bankwallet.core.App.xRateManager, type.coinType)
                is MetricChartType.TradingVolume -> CoinTradingVolumeFetcher(com.starbase.bankwallet.core.App.xRateManager, type.coinType)
                is MetricChartType.MarketGlobal -> MarketGlobalFetcher(com.starbase.bankwallet.core.App.xRateManager, type.metricsType)
                else -> throw IllegalArgumentException("Expected type of MetricChartType, but got different value")
            }

            val service = MetricChartService(com.starbase.bankwallet.core.App.currencyManager.baseCurrency, fetcher)
            val factory = MetricChartFactory(com.starbase.bankwallet.core.App.numberFormatter)
